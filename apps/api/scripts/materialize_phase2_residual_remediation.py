...
def proposal_visual_override(
    ...
    source_correction = dict(row.get("source_text_correction") or {})
    detections = [
        dict(value)
    ...
    if (
        ...
        or not (
            ...
            or (
                str(source_correction.get("source_ocr_text_suggested") or "").strip()
                == approved_text
                and any(
                    str(value.get("text") or "").strip()
                    == str(source_correction.get("encoded_ocr_text") or "").strip()
                    for value in detections
                )
            )
        )
...
def materialize_remediation(
    *,
    ...
    # The frontend proposal filename is mutable and may be regenerated by a
    # retry.  Snapshot the exact approved bytes before recording the authority
    # ref; otherwise a later retry makes an already-materialized remediation
    # appear stale and can trigger duplicate cumulative deltas.
    proposal_snapshot = root / f"phase2_residual_remediation_proposal_{claimed[:12]}.json"
    if proposal_snapshot.is_file():
        if _sha256_file(proposal_snapshot) != _sha256_file(proposal_file):
            raise ResidualRemediationMaterializationError(
                "An immutable residual proposal snapshot already exists with different bytes"
            )
    else:
        _write_json_atomic(proposal_snapshot, proposal)
    payload: dict[str, Any] = {
        ...
        "proposal_ref": {
            "path": proposal_snapshot.name,
            "file_sha256": _sha256_file(proposal_snapshot),
            "proposal_sha256": claimed,
        },
