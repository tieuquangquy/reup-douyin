...
export function OpsRiskPage() {
  ...
  if (!loadedAt && !request.error) {
    return (
      <OpsConsoleShell actions={refreshAction} description={t("opsRisk.description")} title={t("opsRisk.title")}>
        <AsyncContentBoundary skeletonVariant="table" loadingLabel={t("opsRisk.loadingDetail")} status="loading"><span /></AsyncContentBoundary>
      </OpsConsoleShell>
    );
  }
  ...
  if (request.error && !loadedAt) {
    return (
      <OpsConsoleShell actions={refreshAction} description={t("opsRisk.description")} title={t("opsRisk.title")}>
        <AsyncContentBoundary errorState={<OpsState title={t("opsRisk.unavailableTitle")} detail={request.error.message} retry={() => void load()} />} skeletonVariant="table" status="error"><span /></AsyncContentBoundary>
      </OpsConsoleShell>
    );
  }
