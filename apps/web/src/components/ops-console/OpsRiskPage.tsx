...
export function OpsRiskPage() {
  ...
  if (loading && flags.length === 0) return <OpsState title={t("opsRisk.loadingTitle")} detail={t("opsRisk.loadingDetail")} />;
  if (error && flags.length === 0) return <OpsState title={t("opsRisk.unavailableTitle")} detail={error} retry={() => void load()} />;

  return (
    <main className="ops-page">
      <OpsPageHeader title={t("opsRisk.title")} description={t("opsRisk.description")} actions={<button type="button" onClick={() => void load()}>{t("common.refresh")}</button>} />
