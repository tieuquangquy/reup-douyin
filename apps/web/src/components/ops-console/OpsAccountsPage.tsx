...
export function OpsAccountsPage() {
  ...
  if (loading && accounts.length === 0) return <OpsState title={t("opsAccounts.loadingTitle")} detail={t("opsAccounts.loadingDetail")} />;
  if (error && accounts.length === 0) return <OpsState title={t("opsAccounts.unavailableTitle")} detail={error} retry={() => void load()} />;

  return (
    <main className="ops-page">
      <OpsPageHeader title={t("opsAccounts.title")} description={t("opsAccounts.description")} actions={<><a href="/ops/publish-control">{t("opsAccounts.openPublishControl")}</a><button type="button" onClick={() => void load()}>{t("common.refresh")}</button></>} />
