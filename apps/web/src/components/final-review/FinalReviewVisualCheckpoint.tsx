...
export function FinalReviewVisualCheckpoint({ summary, busy, message, onAnalyze, onApprove }: Props) {
  const t = useT();
  ...
  return (
    <section className="final-panel final-visual-checkpoint" aria-label={t("finalReviewVisual.title")}>
      ...
      <p className="final-visual-checkpoint__hint">{t("finalReviewVisual.hint")}</p>
      {message ? <p className="action-message">{message}</p> : null}
      <div className="action-stack">
        <button type="button" onClick={onAnalyze} disabled={busy}>
          {busy ? t("finalReviewVisual.analyzing") : t("finalReviewVisual.analyzeOcr")}
        </button>
        <button type="button" className="primary" onClick={onApprove} disabled={busy || !summary || summary.visual_approved}>
      ...
      {summary ? (
        <div className="final-visual-checkpoint__meta">
          ...
          {summary.visual_approved ? <span className="pill good">{t("finalReviewVisual.approved")}</span> : null}
          ...
          {events.slice(0, 12).map((event, index) => (
            <li key={`${event.start_ms}-${index}`}>
              ...
              <span className="muted">
                y={Math.round(event.y * 100)}% h={Math.round(event.height * 100)}%
              </span>
